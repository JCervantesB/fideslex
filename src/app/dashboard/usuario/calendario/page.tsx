...
import AppointmentsCalendar from "@/components/dashboard/usuario/AppointmentsCalendar";

export default async function UsuarioCalendarioPage() {
  ...
  const itemsToShow = profile.role === "administrador" ? allItems : ownItems;

  return (
    <div className="container mx-auto px-4 py-12 space-y-8">
      <div>
        <h1 className="text-2xl font-semibold">Calendario</h1>
        <p className="mt-2 text-muted-foreground">Todas las citas {profile.role === "administrador" ? "(Admin)" : "(Profesional)"}</p>
      </div>

      <AppointmentsCalendar items={itemsToShow} />
    </div>
  );
}